...
| [companion/](companion/) | Host-side daemon (Python; Windows via WinRT/bleak, macOS via CoreBluetooth/pyobjc) that pushes the active OS layout to the keyboard. Background/autostart installers: [install-windows.ps1](companion/install-windows.ps1) (Scheduled Task), [install-macos.sh](companion/install-macos.sh) (LaunchAgent) | yes |
